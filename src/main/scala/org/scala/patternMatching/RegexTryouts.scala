...
object RegexTryouts extends App {
  ...
  println(multiLineAsSingleTextPattern.findFirstIn(
    src.replace("-(*^4ad", "")))
  ...
  println("[A-Za-z]*(E|e){1,3}(\\w)*".r.
    findAllIn("EEzy eerie007 between2 haloween amazing").mkString(","))
  ...
  println("(\\w+)\\s*=".r.
    findAllMatchIn(sampleCodeSnippet).toList.map(_.group(1)).mkString(","))

  // \\d - digits same as[0-9]
  // \\D - non digits. same as [^0-9]
  println("[0-9]+".r.findAllIn("India won the 1983 and 2011 cricket world cup").mkString(","))
  println("\\d+".r.findAllIn("India won the 1983 and 2011 cricket world cup").mkString(","))
  println("""\d+""".r.findAllIn("India won the 1983 and 2011 cricket world cup").mkString(","))
  ...
  println("<img\\s+.*\\s+src=[\"']([^\"']+)[\\s\\S]+>".r.findFirstMatchIn(sampleHtml)
  .get.group(1))

  println("(?s)<img.*src=[\"']([^\"']+).*>".r.findFirstMatchIn(sampleHtml)
    .get.group(1))
  ...
  println("\\Q**\\E(\\w+)\\Q**\\E".r.findFirstMatchIn(
    """
      | **Markdown** language
      |""".stripMargin
  ).get.group(1))
  ...
  var matches = "(K|C)ite".r.
    findAllMatchIn("Kate Cite Kite Sight").toList
